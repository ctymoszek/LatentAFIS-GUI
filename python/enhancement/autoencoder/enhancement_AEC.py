...
from minutiae.UNet.latent_preprocessing import *
"""
...
class ImportGraph():
    def __init__(self, model_dir):
        ...
        self.weight = get_weights(128, 128,1, sigma=None)
...
class ImageFromFile_AutoEcoder(RNGDataFlow):
    ...
    def get_data(self):
        ...
        for f in self.files:
            ...
            mx = np.random.randint(w-opt.SHAPE)
            my = np.random.randint(h-opt.SHAPE)
            im_label = im[my:my+opt.SHAPE,mx:mx+opt.SHAPE,:]
            im_input = im[my:my+opt.SHAPE,mx:mx+opt.SHAPE,:].copy()

            x = np.random.randint(opt.SHAPE-64)
            y = np.random.randint(opt.SHAPE-64)
            t = np.random.randint(64)
...
class Model(ModelDesc):
    ...
    def reconstruction2(self, imgs):
        ...
        with argscope(Deconv2D, nl=tf.nn.relu, kernel_shape=4, stride=2):
             ...
             l = Deconv2D('deconv7', l, 1, nl=tf.identity)
...
def get_args():
    ...
    parser.add_argument('--data', help='a jpeg directory',default='/home/kaicao/Research/Data/NISTSD14/Image2_jpeg_crop/') #'/home/kaicao/Research/AutomatedLatentRecognition/Patches'
    ...
    parser.add_argument('--log_dir', help='directory to save checkout point', type=str,
                        default='/home/kaicao/Research/AutomatedLatentRecognition/log_AutoEncoder/AutoEncoder_Augmented_2/')
