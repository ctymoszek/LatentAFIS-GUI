...
def evaluation(features_new,features_baseline = None):

    if features_baseline is not None:
        assert(len(features_new['latent']) == len(features_baseline['latent']))




    for i in range(len(features_new['latent'])):
        fpr_1, tpr_1 = feature_evaluation(features_new['latent'][i], features_new['rolled'][i])
        ind = np.where(fpr_1 < 0.001)
        print tpr_1[ind[0][-1]]

        fpr_2, tpr_2 = feature_evaluation(features_baseline['latent'][i], features_baseline['rolled'][i])
        ind = np.where(fpr_2 < 0.001)
        print tpr_2[ind[0][-1]]
...
if __name__=='__main__':
    ...
    model_dirs.append('/home/kaicao/Research/AutomatedLatentRecognition/models/facenet_raw_enhanced_type_1/20171206-093749/')

    patch_types.append(8)
    model_dirs.append('/home/kaicao/Research/AutomatedLatentRecognition/models/facenet_raw_enhanced_type_8/20171207-160445/')

    patch_types.append(11)
    model_dirs.append( '/home/kaicao/Research/AutomatedLatentRecognition/models/facenet_raw_enhanced_type_11/20171207-143926/')
    #
    ...
    if compute_feature:
        ...
        assert(len(latent_img_files) == 258)
        ...
        assert (len(rolled_img_files) == 258)
        rolled_img_files.sort()


        for  patch_type,model_dir in zip(patch_types,model_dirs):
            print patch_type
            print model_dir
            ext = 'texture_patchtype_' + str(patch_type)
            feature_extraction(latent_img_files, matched_latent_minu_files, model_dir,
                           'Data/latent_' + ext + '.npy',
                           patch_type=patch_type)
            feature_extraction(rolled_img_files, matched_rolled_minu_files, model_dir,
                           'Data/rolled_' + ext + '.npy',
                           patch_type=patch_type)

    features_new = {'rolled': [], 'latent': []}
    for patch_type in patch_types:
        ext = 'texture_patchtype_' + str(patch_type)
        features_new['rolled'].append(np.load('Data/rolled_' + ext + '.npy'))
        features_new['latent'].append(np.load('Data/latent_' + ext + '.npy'))


    features_PAMI = {'rolled':[],'latent':[]}
    features_PAMI['rolled'].append(sio.loadmat('Data/PAMI/original_rolled_features_patch_type_1.mat')['features'])
    features_PAMI['rolled'].append(sio.loadmat('Data/PAMI/original_rolled_features_patch_type_8.mat')['features'])
    features_PAMI['rolled'].append(sio.loadmat('Data/PAMI/original_rolled_features_patch_type_11.mat')['features'])
    features_PAMI['latent'].append(sio.loadmat('Data/PAMI/enhanced_latent_features_patch_type_1.mat')['features'])
    features_PAMI['latent'].append(sio.loadmat('Data/PAMI/enhanced_latent_features_patch_type_8.mat')['features'])
    features_PAMI['latent'].append(sio.loadmat('Data/PAMI/enhanced_latent_features_patch_type_11.mat')['features'])

    evaluation(features_new, features_baseline=features_PAMI)
